 ...
 * Date: 3/31/11
 * Time: 10:12 AM
 * Handles display of an EXT-JS Dropdown/Combo box
 */
public class ComboField extends BaseField implements GenericField
{
    ...
    protected LinkedHashMap<String, String> getCustomizationStore(Customization customization)
    {
        LinkedHashMap<String, String> store = new LinkedHashMap<String, String>();

        for (CustomizationItem item : customization.getItem())
        {
            store.put(escapeQuote(item.getText()), item.getId());
        }
        ...
        protected String getCustomizationStoreSelected(Customization customization)
    {
        for (CustomizationItem item : customization.getItem())
        {
            if (item.isSelected()) return item.getId();
        }
