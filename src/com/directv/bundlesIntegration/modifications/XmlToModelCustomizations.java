...
public class XmlToModelCustomizations {
	...
	public static CustomizationModel getModelFromXml(String xml) throws Exception {
		...
		if (doc == null || doc.getResponse() == null)
			throw new Exception("Empty customization document received");
	...
	private static void translateModel(ResponseDocument doc, CustomizationModel model) throws Exception {
		...
		for (NameValuePairEntity nvp : doc.getResponse().getCustomData().getParamKVPairArray()) {
			if (nvp.getName() != null && nvp.getName().equalsIgnoreCase(PUNCH_OUT_URL) && nvp.getValueArray() != null
					&& nvp.getValueArray()[0] != null) {
				model.setPunchOutURL(nvp.getValueArray()[0]);
			}
		}
		if (model.getPunchOutURL() == null)
			throw new Exception("Empty customization punch out URL received");
	...
	private static void translateGroupList(ProcessNonVideoCustomizationsResponseEntity.CustomizationGroupList docGroupList,
			CustomizationGroupList modelGroupList, CustomizationModel model) throws Exception {
	...
	private static void translateGroup(NonVideoCustomizationGroupEntity[] docGroupEntity, ArrayList<CustomizationGroup> modelGroupEntity,
	...
	private static void translateCustomizationList(NonVideoCustomizationGroupEntity.CustomizationList docCustomizationList,
			ArrayList<CustomizationList> modelCustomizationList, CustomizationModel model) {
	...
	private static void translateCustomization(NonVideoCustomizationEntity docCustomization, CustomizationList modelList, CustomizationModel model) {
		...
		if (docCustomization.getImmediateSubmissionFlag())
			model.setImmediateSubmissionFlag(true);
